...
## 左右を切り替える（単体使用時）
標準では左手用のQWERTYの配列になっていて、右手用で作るとキーマップがずれています。側面ユニットに一番近いキーを押しながらピンヘッダに一番近いキーを押すと右手用のYUIOPの配列に切り替えることができます。
![](../img/custom/lr.png)  
...
|レイヤーの変更と修飾キー両方の機能を持たせる|LM(レイヤー番号、修飾キーコード)|
...
### 保存と復元
VIAで設定したキーマップは保存、復元することができます。ファームウェアを入れ替えると初期化されるので、保存しておくと便利です。
![](../img/custom/mouse.jpg)  
