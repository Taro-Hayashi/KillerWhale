...
- 単体使用 [tarohayashi_killerwhale_solo_rgblayer.uf2
...
- 左右分割 両手ボール [tarohayashi_killerwhale_duo_defaultlayer.uf2
...
- 天面ユニットで3か所だけLEDの色が違う
  - NumLock、ScrollLock、CapsLockがオンになると色が変わります。 また、レイヤーの状態によっても一部の色が変化します。煩わしい場合はRGBレイヤーなしのファームウェアに変更してください。
- 側面ユニットのキー以外の機能が動かない
- 分割キーボードでキーマップが左右逆
