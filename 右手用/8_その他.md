...
## ブートローダーモードへの入り方
1. Raspberry Pi PicoのBOOTボタンを押しながらUSBケーブルを接続する
2. UtilレイヤーのQK_BOOTを押す（カスタマイズ参照）
3. USBに一番近いキー（5、6）を押しながらUSBケーブルを接続する

1、3でファームウェアを更新すると設定も初期化されます。
## VIA用JSONファイル
...
- 左右分割 デフォルトキーマップ [killer_whale_duo.layout.json](https://github.com/Taro-Hayashi/KillerWhale/releases/download/0.21.4.1/killer_whale_duo.layout.json)
...
- ファームウェアを入れ替えたらキーマップがおかしくなった
  - 前のファームウェアのキーマップが保存されている可能性があります。ブートローダーへの入り方を参考に設定を初期化する、保存したキーマップやサンプルのキーマップを読み込む、または↓をお試しください。
- 各種キーが効かない
- 設定が保存されない
- 発光パターンがおかしい
